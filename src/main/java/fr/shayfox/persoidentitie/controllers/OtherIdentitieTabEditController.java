...
public class OtherIdentitieTabEditController implements Initializable {
    @FXML
    public JFXTextField Owner, NameLabel, PrenameLabel, MarriedNameLabel, BornCityLabel, DeathCityLabel,
    ...
    @FXML
    public JFXListView<String> FamiliesList, OtherNameList, DefaultList, QualityList, PowersList;
    @FXML
    ...
    private void initComponent() {
        ...
        owner = filename.split("\\)")[0].replace("(", "");
        Owner.setText(owner);
        ...
        if(!charactere.StringVar.get("BornDate").equalsIgnoreCase("Non Renseigné") && !charactere.StringVar.get("BornDate").equalsIgnoreCase("dd/MM/yyyy") && isValidFormat(charactere.StringVar.get("BornDate"))) {
            BornDateLabel.setText(String.valueOf(LocalDate.parse(charactere.StringVar.get("BornDate"), formatter).format(formatter)));
        }else{
        ...
        if(!charactere.StringVar.get("DeathDate").equalsIgnoreCase("Non Renseigné") && !charactere.StringVar.get("DeathDate").equalsIgnoreCase("dd/MM/yyyy") && isValidFormat(charactere.StringVar.get("DeathDate"))) {
            DeathDateLabel.setText(String.valueOf(LocalDate.parse(charactere.StringVar.get("DeathDate"), formatter).format(formatter)));
        }else{
        ...
        AddFamiliesButton.setOnAction(e -> {
                ...
                tab.remove("images");
                ArrayList<String> array = new ArrayList<String>(FamiliesList.getItems().size());
    ...
    private void Save() {
        ...
        /*TODO################When Name Changed#################*/
        boolean changename = !filename.equalsIgnoreCase("("+owner+")"+PrenameLabel.getText()+" "+NameLabel.getText());

        if(changename){
            filename = "("+owner+")"+PrenameLabel.getText() +" "+ NameLabel.getText();
            String oldname = charactere.StringVar.get("Prename") + " " + charactere.StringVar.get("Name");
            ...
            if(newname.matches("^[a-zA-Z]+[ \\-']?[[a-z]+[ \\-']?]*[a-zA-Z \\-]+$")) {
                ...
                YamlFile yamlFile = new YamlFile(appdata + "/PersoIdentitie/fiches/"+owner+".yml");
                ArrayList<String> array = new ArrayList<>();
                try {
                    yamlFile.load();
                    for (String arg:yamlFile.getStringList("OtherIdentities")) {
                        if(!arg.equalsIgnoreCase(oldname))array.add(arg);
                    }
                    array.add(newname);
                    yamlFile.set("OtherIdentities", array);
                    yamlFile.save();
                } catch (Exception e) {
                    e.printStackTrace();
                }
